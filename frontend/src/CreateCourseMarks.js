...
/** Label shown in objective dropdown — description only (not title). */
const objectiveDropdownLabel = (topic) => {
...
const CreateCourseMarks = () => {
  ...
  const topics = useMemo(
    () => (Array.isArray(resolvedTopics) ? resolvedTopics : []),
    [resolvedTopics]
  );
  ...
  const handleCreateCourse = async () => {
    ...
    if (topics.length === 0) {
      setCreateError('No objectives available. Go back and select objectives for this course.');
      return;
    }
  ...
  return (
    <div className="create-course-marks-container">      <div className="create-course-marks-content">
        ...
        <p className="create-course-marks-hint">
          ...
          {' '}Objective dropdowns show the selected objectives (Description from Objectives, not Title). Hover an option to read the full text.
        </p>
                      ...
                      {topics.map((t) => {
                        ...
                        return (
                          <option
                            key={t.topicKey}
                            value={t.topicKey}
                            title={String(t.description || '').trim() || t.topicName}
                          >
                            {label}
                          </option>
                        );
